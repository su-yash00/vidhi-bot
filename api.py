...
"""
STEP 5: FastAPI Backend for Legal RAG with Citations (Gemini Version)
"""
from fastapi import FastAPI, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel
...
# Initialize FastAPI
app = FastAPI(
    title="Nepali Legal RAG API with Citations (Gemini)",
    description="Query Nepali legal documents using Gemini AI with proper citations",
...
def generate_answer_with_citations(query: str, documents: List[str], metadatas: List[dict]):
    """Generate answer with proper inline citations using Gemini"""
    # Build context with citation markers
    context_parts = []
    for i, (doc, meta) in enumerate(zip(documents, metadatas)):
        # Create citation marker with all available info
    ...
    prompt = f"""You are a legal assistant for Nepali law specialized in providing accurate citations.
...
Provide a comprehensive answer with inline citations for every claim. Use the exact citation format shown in the context. Answer based ONLY on provided documents."""
...
async def root():
    return {
        "message": "Nepali Legal RAG API with Citations (Gemini)",
        "version": "2.0.0-gemini",
